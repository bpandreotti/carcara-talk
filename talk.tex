...
\begin{frame}[b, fragile]
  \frametitle{Adding resolution pivots}
  \only<1>{
    \begin{itemize}
      \item As we saw, since \texttt{resolution} steps do not indicate the pivots
      used, checking this rule can be tricky
      \vitem We developed an elaboration procedure that finds these pivots (using
      the greedy algorithm) and adds them as arguments to the step
    \end{itemize}
  }
  \only<2->{
    \begin{itemize}
      \item<2-> For each pivot, we provide two arguments: the pivot term itself, and
      a boolean indicating its polarity
      \vitem<3> Since this uses the greedy (incomplete) algorithm, not all
      resolution steps can be elaborated with this technique
    \end{itemize}
  }
  \vfill
  \begin{minted}[autogobble, fontsize=\footnotesize, frame=single, escapeinside=||]{smtlib2.py -x}
    ...
    (step t4 (cl f) :rule ...)
    (step t5 (cl b d e) :rule resolution :premsies (t1 t2 t3 t4)
        |\chighlight{yellow!60}{:args (a true c true f false)}|)
  \end{minted}
  \bigskip
\end{frame}
